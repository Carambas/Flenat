...
if (!$news_name or !$news_text or !$news_text_full) {
    ...
    if ($is_news != 1) {
        ...
        $db->query("INSERT INTO news SET name = '$news_name', text = '$news_text', text_full = '$news_text_full', date = '$_TIME', author='{$user_id['name']}'");
        info('Информация', 'Новость успешно добавлена.');
        if ($config['cache'] == '1') {
            @clear_all();
        } 
    } 
